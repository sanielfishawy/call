...
# end
# 
# named_scope :status_active, { :conditions => { :status => :active} }
# named_scope :status_not_active, {:conditions => ["status <> ?",:active]}
# 
# However, if the :primary option is set to true, then the prefix is taken out of the named scope, and we just have:
# named_scope, :active, { :conditions => { :status => :active} }
# named_scope, :not_active, :{:conditions => ["status <> ?",:active]
# 
...
module EnumHandler
  ...
  module ClassMethods
    ...
    def define_enum( attribute, values_hash, options = {})
      ...
      # Unfortunately, we lose the sort order (will be fixed with Ruby 1.9)
      ...
      class_eval <<-END, __FILE__, __LINE__
        ...
        # def self.db_#{attribute}_code(symbol_value,eval_context='')
        #   symbol_value = symbol_value.to_sym
        #   validate_#{attribute}_value(symbol_value,eval_context) if self.attribute_options['#{attribute.to_s}'][:strict]
        #   self.db_codes[eval_context]['#{attribute}'][symbol_value]
        # end
        # 
        # def self.db_#{attribute}_value(code,eval_context='')
        #   self.db_codes[eval_context]['#{attribute}'].key(code)
        # end
      ...
      if scope_keyword
        (values_hash.keys + (options[:sets] ? options[:sets].keys : [])).each do |v|
          if options[:primary]
            class_eval <<-EVAL_END, __FILE__, __LINE__
            #{scope_keyword} v.to_sym, :conditions => {"#{attribute}".to_sym => db_code("#{attribute}",v,true)}
            #{scope_keyword} "not_#{v}".to_sym, :conditions => ["#{attribute} <> ?",db_code("#{attribute}",v,true)]
            EVAL_END
          else
            class_eval <<-EVAL_END, __FILE__, __LINE__
            #{scope_keyword} "#{attribute}_#{v}".to_sym, :conditions => {"#{attribute}".to_sym => db_code("#{attribute}",v,true)}
            #{scope_keyword} "#{attribute}_not_#{v}".to_sym, :conditions => ["#{attribute} <> ?",db_code("#{attribute}",v,true)]
            EVAL_END
          end
        end
      end

      # See if we're to generate appropriate finders
      if options[:generate_methods] 
        (values_hash.keys + (options[:sets] ? options[:sets].keys : [])).each do |v|
          if options[:generate_methods].include?(:finders)
            class_eval <<-EVAL_END, __FILE__, __LINE__
              class<<self
                def with_#{attribute}_#{v} options={}
                  find(:all,options.merge(:conditions => {'#{attribute}'.to_sym => '#{v}'.to_sym})) 
                end
                def with_#{attribute}_not_#{v} options={}
                  find(:all,options.merge(:conditions => ['#{attribute} <> ?','#{v}'.to_sym])) 
                end
              end
            EVAL_END
          end
          if options[:generate_methods].include?(:counters)
            class_eval <<-END, __FILE__, __LINE__
              class<<self
                def count_with_#{attribute}_#{v}(options={})
                  count(:conditions => {'#{attribute}'.to_sym => '#{v}'.to_sym}) 
                end
                def count_with_#{attribute}_not_#{v}
                  count(:conditions => ['#{attribute} <> ?', '#{v}'.to_sym]) 
                end
              end
            END
          end
